...
from mcp_tool_loader import get_mcp_tools
import asyncio
...
async def test():
    ...
    tools=await get_mcp_tools(["call_search_agent", "call_reason_agent"]),
    prompt=orchestrator_prompt,
)

    result = await agent.ainvoke({"messages": "What is otto von guerickes birthday regarding wikipedia?"})
    print(result)
